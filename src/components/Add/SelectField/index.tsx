...
const SelectFieldFunction: React.ForwardRefRenderFunction<HTMLDivElement, SelectFieldProps> = ({ fields, onSelectField }, ref) => {
  ...
  const items = fields.filter((item) => item.label.toLowerCase().includes(search.toLowerCase()))
  return (
    <div className="rsql-box rsql-select-field" ref={ref}>
      <input value={search} onChange={(e) => setSearch(e.target.value)} className="rsql-input" name="rsql-field-search" placeholder="Filter by..." />
      <div>
        {items.map((item) => {
